...
interface PatientData {
    ...
    gender: number;
    family_history_myopia: number;
    outdoor_time: number;
    screen_time: number;
    previous_myopia_control: number;
    initial_power_re: number;
    initial_power_le: number;
    initial_axial_length_re: number;
    initial_axial_length_le: number;
    stellest_wearing_time: number;
}

interface PredictionResult {
    patient_name?: string;
    ensemble_prediction: {
        will_benefit: boolean;
        probability: number;
...
class StellestPredictor {
    private currentTab: string = 'predict';
    ...
    private displayModelInfo(info: ModelInfo): void {
    ...
    private collectFormData(): Partial<PatientData> {
        const form = document.getElementById('predictionForm') as HTMLFormElement;
    ...
    private validateFormData(data: Partial<PatientData>): boolean {
        ...
        requiredFields.forEach(field => {
            ...
            } else if (field !== 'patient_name' && isNaN(data[field] as number)) {
                missingFields.push(field.replace(/_/g, ' '));
            }
        });
    ...
    private displayResults(result: PredictionResult): void {
    ...
    private renderEnhancedAnalytics(analytics: any): string {
        return `
            <div class="card mt-6">
                <div class="card-header">
    ...
    private renderOpenAIAnalysis(analysis: any): string {
        return `
            <div class="card mt-6">
                <div class="card-header">
